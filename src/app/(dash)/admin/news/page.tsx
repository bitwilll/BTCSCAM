    ...
    <div className="space-y-8">
      ...
      <section className="border border-line bg-paper-2 p-5">
        ...
        <div className="mt-4 divide-y divide-line">
          {sources.length === 0 && <p className="mono text-sm text-ink-500">No sources yet — add defaults above.</p>}
          {sources.map((s) => (
            <div key={s.id} className="flex items-center justify-between gap-4 py-2">
              <div className="min-w-0">
                <div className="font-bold text-ink">{s.name}</div>
                <div className="mono text-[11px] text-ink-500 truncate">{s.feedUrl}</div>
                {s.lastFetchedAt && <div className="mono text-[10px] text-ink-400">last fetched {timeAgo(s.lastFetchedAt)}</div>}
              </div>
      ...
      <div className="flex gap-2">
        {STATUS_TABS.map((t) => (
          <Link
            ...
            className={`kicker px-3 py-2 border ${t === status ? "bg-ink text-paper border-ink" : "border-line-strong text-ink-600 hover:bg-panel"}`}
          >
            {t} ({countFor(t)})
          </Link>
        ))}
      ...
      <div className="space-y-4">
        {items.length === 0 && (
          <div className="border border-dashed border-line-strong p-8 text-center mono text-sm text-ink-500">
            No {status} items. Click “Fetch latest news”.
          </div>
        )}
        {items.map((it) => (
          <article key={it.id} className="border border-line bg-paper p-4 flex gap-4">
            <div className="w-32 shrink-0 hidden sm:block">
              {it.imageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={it.imageUrl} alt="" className="w-32 h-24 object-cover border border-line" />
              ) : (
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1 flex-wrap">
                <span className="kicker text-btc-dark">{it.sourceName}</span>
                {looksScammy(`${it.title} ${it.summary ?? ""}`) && <Tag tone="red">Scam-relevant</Tag>}
                {it.publishedAt && <span className="mono text-[10px] text-ink-400 uppercase">{byline(it.publishedAt)}</span>}
              </div>
              <h3 className="font-extrabold text-ink leading-tight">
                <a href={it.url} target="_blank" rel="noopener noreferrer" className="hover:text-btc-dark">{it.title} ↗</a>
              </h3>
              {it.summary && <p className="text-sm text-ink-600 mt-1 line-clamp-3">{it.summary}</p>}
              <div className="mt-3">
                {status === "dismissed" ? (
                  <RestoreButton id={it.id} />
                ) : status === "published" ? (
                  it.pushedArticleId ? (
                    <Link href={`/admin/articles/${it.pushedArticleId}/edit`} className="kicker text-btc-dark hover:text-ink">Edit draft →</Link>
                  ) : (
                    <span className="mono text-[11px] text-ink-400">pushed</span>
                  )
                ) : (
